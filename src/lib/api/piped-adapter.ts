...
export class PipedAdapter implements ApiAdapter {
  async search(query: string): Promise<SearchChannelsResult[]> {
    const res = await Piped.search(query, { filter: Piped.FILTER_CHANNELS })
    if (!res.ok) return []
  ...
  async fetchChannel(id: string, reload?: boolean): Promise<Channel> {
    ...
    const streams = body.relatedStreams ?? body.content
  ...
  async fetchPlaylist(id: string, _reload?: boolean): Promise<Channel> {
    ...
    return {
      ...
      videos: this.mapVideos(body.relatedStreams)
    }
  }
